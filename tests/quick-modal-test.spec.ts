...
test("Quick modal test", async ({ page }) => {
  ...
  if (cardCount > 0) {
    ...
    console.log(
      `📄 Testing first card: ${cardInfo.title} at ${cardInfo.company}`
    );
    ...
    const modalCount = await page
      .locator('[role="dialog"], .modal, .backdrop, div[class*="modal"]')
      .count();
    ...
    if (modalCount > 0) {
      ...
      const overlaysCount = await page
        .locator('div[class*="overlay"], div[class*="backdrop"]')
        .count();
