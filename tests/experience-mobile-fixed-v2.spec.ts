...
test.describe("Experience Section Mobile - Fixed", () => {
  ...
  async function navigateToExperienceSection(page: any) {
    ...
    const experienceButton = page.locator('nav div:last-child').locator('button').filter({ hasText: "Experiencia" });
  ...
  test("should successfully navigate to experience section", async ({ page }) => {
  ...
  test("should detect navigation overlay issues", async ({ page }) => {
    ...
    if (cardCount > 0) {
      ...
      if (cardBox && navBox) {
        ...
        if (overlap) {
          console.log("🚨 CRITICAL: Navigation is overlapping with experience cards!");
          console.log("Navigation z-index should be lower or pointer-events should be 'none' on content area");
        }
      }
      ...
      try {
        ...
        console.log("❌ Card is NOT clickable - navigation is blocking:", error.message);
  ...
  test("should identify the root cause of mobile UX issues", async ({ page }) => {
    console.log("🔍 Diagnosing mobile UX issues...");
    
    await navigateToExperienceSection(page);
    
    // Check all clickable elements on the page
    const allClickable = page.locator('a, button, [onclick], [role="button"], .cursor-pointer');
      ...
      for (let i = 0; i < Math.min(mobileCardCount, 2); i++) {
        ...
        if (cardBox) {
          ...
          try {
            ...
            console.log(`❌ Mouse click on card ${i + 1} failed:`, error.message);
          }
        }
      }
    }
