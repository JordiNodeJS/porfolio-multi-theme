...
// Declare global window type for click events
declare global {
  interface Window {
    clickEvents: Array<{
      target: string;
...
test.describe("Experience Section Mobile Issues", () => {
  ...
  test("should not trigger unexpected scroll when clicking experience cards on mobile", async ({
    ...
    // Navegar a la sección de experiencia usando el texto en inglés
    await page
      .locator("button")
    ...
    if ((await mobileCards.count()) > 0) {
      ...
      if (projectsSectionPosition) {
        ...
        const scrolledToProjects =
          Math.abs(
            finalScrollY - (projectsSectionPosition.y + initialScrollY)
          ) < 100;
  ...
  test("should open modal correctly when clicking experience cards", async ({
    ...
    if ((await mobileCards.count()) > 0) {
      ...
      const modalContent = page
        .locator(".fixed.right-0, .fixed.left-0")
        .filter({
          hasText: /achievements|logros/i,
        });
  ...
  test("should identify conflicting event handlers", async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      document.addEventListener(
        ...
        (e) => {
          ...
          if (target) {
            (window as any).clickEvents.push({
              target:
                target.tagName +
                (target.className
                  ? "." + target.className.replace(/\s+/g, ".")
                  : ""),
              timestamp: Date.now(),
    ...
    if ((await mobileCards.count()) > 0) {
      ...
      const events = await page.evaluate(() => (window as any).clickEvents);
  ...
  test("should not have overlapping clickable elements", async ({ page }) => {
    ...
    const clickableElements = await page
      .locator('a, button, [onclick], [role="button"]')
      .all();

    interface OverlapInfo {
      element1: { tag: string; class: any; text: string | undefined };
      element2: { tag: string; class: any; text: string | undefined };
    ...
    for (let i = 0; i < clickableElements.length; i++) {
      for (let j = i + 1; j < clickableElements.length; j++) {
        ...
        if (box1 && box2) {
          ...
          if (overlap) {
            const element1Info = {
              tag: await element1.evaluate((el) => el.tagName),
              class: await element1.evaluate((el) => el.className),
              text: await element1.textContent(),
            };
            const element2Info = {
              tag: await element2.evaluate((el) => el.tagName),
              class: await element2.evaluate((el) => el.className),
              text: await element2.textContent(),
            };
    ...
    // Filter overlaps related to experience cards
    const experienceOverlaps = overlaps.filter(
      (overlap) =>
    ...
    console.log(
      `Found ${experienceOverlaps.length} overlapping clickable elements in experience section:`,
      experienceOverlaps
    );

    // Experience cards should not have significant overlaps
    expect(experienceOverlaps.length).toBe(0);
  });
  ...
  test("should handle mobile viewport correctly", async ({ page }) => {
    ...
    console.log(`Mobile viewport: Found ${mobileCardCount} mobile cards`);

    // Should have mobile cards visible
    expect(mobileCardCount).toBeGreaterThan(0);

    // Check if desktop cards are hidden
    const desktopCards = page.locator(".hidden.md\\:flex");
    const desktopCardCount = await desktopCards.count();

    console.log(`Mobile viewport: Found ${desktopCardCount} desktop cards`);

    // Desktop cards should be hidden on mobile
    for (let i = 0; i < desktopCardCount; i++) {
      const card = desktopCards.nth(i);
