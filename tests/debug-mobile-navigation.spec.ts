...
test.describe("Mobile Navigation Debugging", () => {
  ...
  test("Debug navigation structure in mobile", async ({ page }) => {
    ...
    const hamburgerButton = page
      .locator("nav button")
      .filter({ hasText: /^$/ })
      .last(); // Button with no text (just icon)
    ...
    if (hamburgerExists > 0) {
      ...
      if (isHamburgerVisible) {
        ...
        const experienceButton = page.locator("nav button", {
          hasText: "Experience",
        });
        ...
        for (let i = 0; i < experienceCount; i++) {
          ...
          console.log(
            `Experience button ${i}: visible=${isVisible}, boundingBox=`,
            boundingBox
          );
        }
      }
    ...
    const allExperienceButtons = page.locator("button", {
      hasText: "Experience",
    });
    ...
    for (let i = 0; i < allExperienceCount; i++) {
      ...
      console.log(
        `All Experience button ${i}: visible=${isVisible}, boundingBox=`,
        boundingBox,
        "computedStyle=",
        computedStyle
      );
  ...
  test("Test mobile menu interaction step by step", async ({ page }) => {
    ...
    const mobileExperienceButton = page
      .locator("nav button")
    ...
    if (buttonCount > 0) {
      ...
      console.log(
        `First Experience button - visible: ${isVisible}, enabled: ${isEnabled}`
      );
