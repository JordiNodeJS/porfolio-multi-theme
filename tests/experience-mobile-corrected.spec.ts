...
test.describe("Experience Section Mobile Issues", () => {
  ...
  test("should not trigger unexpected scroll when clicking experience cards on mobile", async ({
    ...
    // Navegar a la sección de experiencia usando el botón correcto
    await page
      .locator("button")
  ...
  test("should not have overlapping clickable elements", async ({ page }) => {
    ...
    if (overlaps.length > 0) {
      console.log(
        "Found overlapping clickable elements:",
    ...
    console.log(
      `Found ${experienceOverlaps.length} overlaps in experience section`
    );
  });
});
