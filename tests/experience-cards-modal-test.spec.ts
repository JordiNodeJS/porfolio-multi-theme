...
test.describe("Experience Cards Modal Test - Mobile", () => {
  ...
  async function navigateToExperienceSection(page: any) {
    ...
    const isHamburgerVisible = await hamburgerButton
      .isVisible()
      .catch(() => false);
    ...
    if (isHamburgerVisible) {
      ...
      const experienceButton = page
        .locator("nav div:last-child button")
        .filter({
      ...
      console.log(
        `🔍 Encontrados ${buttonCount} botones de Experiencia en el menú móvil`
      );
      ...
      const experienceLink = page
        .locator('a[href="#experience"], button')
        .filter({
          hasText: /Experiencia|Experience/i,
        })
        .first();
    ...
    const isHeadingVisible = await experienceHeading
      .isVisible()
      .catch(() => false);
    if (isHeadingVisible) {
      ...
      console.log(
        "⚠️ No se puede verificar el heading de experiencia, continuando..."
      );
    }
  }

  test("should test all mobile experience cards for modal functionality", async ({
    page,
  }) => {
    ...
    for (let i = 0; i < cardCount; i++) {
      ...
      try {
        ...
        const modal = page
          .locator('[role="dialog"], .modal, .overlay, .backdrop')
          .first();
        ...
        if (isModalVisible) {
          ...
          const possibleModals = await page
            .locator(
        ...
        console.log(
          `❌ Error haciendo click en tarjeta ${i + 1}:`,
          error.message
        );
  ...
  test("should identify specific cards that don't show modal", async ({
    page,
  }) => {
    await navigateToExperienceSection(page);
    ...
    for (let i = 0; i < cardCount; i++) {
      ...
      const cardData = await card.evaluate((el) => {
        ...
        return {
          ...
          eventListeners: el.getEventListeners
            ? Object.keys(el.getEventListeners())
            : [],
        };
      });
      ...
      try {
        ...
        const modalExists = await page
          .locator('[role="dialog"], .modal, div[class*="modal"]')
          .isVisible()
          .catch(() => false);
