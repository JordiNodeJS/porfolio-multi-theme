...
test.describe('Navegación - Problema de Offset', () => {
  ...
  test('debería verificar que los títulos de sección se ocultan parcialmente tras la navegación fija', async ({ page }) => {
    // Esperar a que la navegación esté visible
    await page.waitForSelector('nav', { timeout: 5000 });
    
    // Obtener la altura de la navegación
    const navigation = page.locator('nav');
    const navHeight = await navigation.evaluate(el => el.getBoundingClientRect().height);
    console.log('Altura de navegación:', navHeight);

    // Hacer clic en "Experience" desde la navegación
    await page.click('nav button:has-text("Experiencia"), nav button:has-text("Experience")');
    ...
    if (titleBounds) {
      ...
      if (titleBounds.y < navHeight) {
        console.log('❌ PROBLEMA DETECTADO: El título está oculto parcialmente detrás de la navegación');
        console.log(`Título en Y: ${titleBounds.y}, Navegación altura: ${navHeight}`);
    ...
    for (const section of sectionsToTest) {
      ...
      await page.click(`nav button:has-text("${section.navText}"), nav button[href*="${section.selector.replace('#', '').replace(' h2', '')}"]`);
      await page.waitForTimeout(1000);
      ...
      if (await sectionTitle.isVisible()) {
        ...
        if (bounds && bounds.y < navHeight) {
          console.log(`❌ ${section.navText}: Título oculto parcialmente (Y: ${bounds.y}, Nav: ${navHeight})`);
        }
      }
    }
  });

  test('debería mostrar el offset visual necesario para cada sección', async ({ page }) => {
    // Tomar screenshot inicial
    await page.screenshot({ path: 'test-results/navigation-offset-before.png', fullPage: true });
