...
test.describe("Mobile Experience Cards - Post Navigation Fix", () => {
  test("should confirm experience cards are now clickable after navigation fix", async ({ page }) => {
    await page.goto("/");
    await page.waitForLoadState("networkidle");
    
    console.log("🔧 Testing mobile experience cards after navigation pointer-events fix...");
    
    // Step 1: Navigate to experience section using mobile menu
    console.log("📱 Opening mobile menu...");
    const hamburgerButton = page.locator('nav button').filter({ has: page.locator('svg') }).first();
    await hamburgerButton.click();
    await page.waitForTimeout(500);
    
    console.log("🎯 Clicking Experience in mobile menu...");
    const experienceButton = page.locator('button').filter({ hasText: "Experiencia" }).nth(1);
